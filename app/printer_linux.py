...
def is_online() -> bool:
    """True si la cola existe y no está pausada/con error (sin abrir el
    dispositivo USB: solo lee el estado que ya sabe CUPS)."""
    if not config.PRINTER_NAME:
        return False
    result = _run(["lpstat", "-p", config.PRINTER_NAME])
    if result.returncode != 0:
        return False
    out = result.stdout.lower()
    if "disabled" in out or "stopped" in out:
        return False
    return "idle" in out or "printing" in out


def status_message() -> str:
    if not config.PRINTER_NAME:
        return "PRINTER_NAME no configurado en .env"
    result = _run(["lpstat", "-p", config.PRINTER_NAME])
    if result.returncode != 0:
        return f"no encontré la cola '{config.PRINTER_NAME}'"
    first_line = result.stdout.strip().splitlines()
    return first_line[0] if first_line else "sin información"
